...
Version = paste0(Sys.Date(),"_", SampleType, "_", CCDBType, "_Targene_Heatmap_V3")
...
#   rm(LR_Tar.df, df, df1, TarGeneM.set, TarGeneH.set)
  ...
  TarGene.df <- read.delim("HumanRNA_TargetGene.txt",header=T, stringsAsFactors = FALSE)
    ...
    ## Mouse Genes
    source("FUN_HSsymbol2MMsymbol.R")
    df <- TarGene.df$Gene %>% as.data.frame()
    colnames(df) <- "Gene"
    df1 <- HSsymbol2MMsymbol(df,"Gene")
    TarGeneM.set <- df1$MM.symbol
    TarGene <- c(TarGene.df$Gene,TarGeneM.set) %>% unique()
...
# }
  ...
  SummaryTable.df <- SummaryTable_Sub.df
...
##***************************************## 20221123## TarGene_Ori.set <- TarGene
### FDR df Setting
