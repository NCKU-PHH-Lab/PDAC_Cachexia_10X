...
##### 06 Cell type annotation  #####
  ...
  Cell_Type_Order.set <- c("T", "CD4+T", "CD8+T", "B" , "Mac1", "Mac2", "Mac3", "Mast", "NK", "Neu", "Ery")
  
    
  PBMC.combined$celltype <- Idents(PBMC.combined)
  PBMC.combinedTTT <- PBMC.combined[,!grepl("Other",PBMC.combined$celltype)]
...
##### 07 Count Cell number  #####
  Pheno.df <- data.frame(sample = PBMC.combined@meta.data[["sample"]],celltype = PBMC.combined@meta.data[["celltype"]],
                         Cachexia = PBMC.combined@meta.data[["Cachexia"]],Sex = PBMC.combined@meta.data[["Sex"]])
...
##### 08_1 Find CCmarker in different Cell type and VolcanoPlot (SSA) ########
  ...
  ##-------------- Find Marker gene in Male --------------##
    ...
    for(i in c(1:length(CellType.list))){ 
      try({
      CCMarker_Male.lt[[i]] <- Find_Markers(PBMC.combined, 
  ...
  ##-------------- Find Marker gene in Female --------------##
    ...
    for(i in c(1:length(CellType.list))){ 
      try({
        CCMarker_Female.lt[[i]] <- Find_Markers(PBMC.combined, 
...
##### 08_2 Find CCmarker in different Cell type and VolcanoPlot (SPA) ########
  ...
  dir.create(paste0(Save.Path,"/PBMC_SPA_FindMarkers"))
    ...
    for(i in c(1:length(CellType.list))){ 
      try({
        CCMarker_SPA.lt[[i]] <- Find_Markers(PBMC.combined, 
