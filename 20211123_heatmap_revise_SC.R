...
raw.df <- read.csv(
  # "D:/KLC/### Datasets/Cachexia_differential-gene-heatmap/20210923/20210921_SC/SC_Cachexia.Marker_Sum_LogFC_M.txt",
  #"D:/Dropbox/Chang Charlene/##_GitHub/0-R/##_PHH_Lab/2021_Cachexia/20211121_SC/SC_Cachexia.Marker_Sum_LogFC_M.txt",
  "D:/Dropbox/##_GitHub/0-R/##_PHH_Lab/2021_Cachexia/20211121_SC/SC_Cachexia.Marker_Sum_LogFC_M.txt",
  sep = "\t"
)

Sig.df <- read.csv(
  # "D:/KLC/### Datasets/Cachexia_differential-gene-heatmap/20210923/20210921_SC/Cachexia_SC_GSEAforCheck.txt",
  #"D:/Dropbox/Chang Charlene/##_GitHub/0-R/##_PHH_Lab/2021_Cachexia/20211121_SC/Cachexia_SC_GSEAforCheck.txt",
  "D:/Dropbox/##_GitHub/0-R/##_PHH_Lab/2021_Cachexia/20211121_SC/Cachexia_SC_GSEAforCheck.txt",
  
  sep = "\t"
)
SC_phenotype <- paste(sort(raw.df$CellType),rep(c("F","M"),times = length(raw.df$Sex)/2), sep = "_")
...
Anno.df$celltype <- factor(Anno.df$celltype , levels = unique(Anno.df$celltype))
...
Pos_ES <- Pos_ES[match(SC_phenotype, Pos_ES$pheno_type),]
Neg_ES <- Anno.df[Anno.df$GeneType=="Neg",] 
Neg_ES <- Neg_ES[match(SC_phenotype, Neg_ES$pheno_type),]
...
fig1 <- rowAnnotation(
  ...
  Heatmap(
    mt,
    column_title = "SC_cachexia marker",
...
write.table(
  ...
  file = paste0(getwd(),"/20211121_SC/SC_PairedMarker.tsv"),
  sep = "\t",
  quote = F,
  row.names = F
)
