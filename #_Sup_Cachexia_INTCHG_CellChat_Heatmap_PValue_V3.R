...
for (i in 1:nrow(statistics_FDR.mtx)) {
  if(sum(statistics_FDR.mtx[i, ] <= SetFDRThr) >= 1){
...
for (i in 1:nrow(statistics_LogFC.mtx)) {
  if(sum(abs(statistics_LogFC.mtx[i, ]) >=  SetLogFCThr) >= 1){
...
Heatmap(
  ...
  layer_fun = function(j, i, x, y, width, height, fill)
  {
    ...
    l2 = v1 <= SetFDRThr & abs(v2) >= SetLogFCThr
    grid.points(x[l2], y[l2], pch = pch2, size = unit(pchSize2, "mm"))

    l3 = abs(v2) >= SetLogFCThr & v1 > SetFDRThr
    grid.points(x[l3], y[l3], pch = pch3, size = unit(pchSize3, "mm"))
  }
) -> P.Heatmap_LogFC
