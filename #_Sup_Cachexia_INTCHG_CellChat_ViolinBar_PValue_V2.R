...
## Ref: https://psyteachr.github.io/introdataviz/advanced-plots.html
## Ref: https://psyteachr.github.io/msc-conv/visualisation.html
...
if(SampleType == "PBMC"){
  ...
  # Clean up
  rm(list=setdiff(ls(), c("scRNA.SeuObj","SampleType","Save.Path","CCDBType","CellType.Order","CCDBType.set")))
...
##### For Different DB ####
  for (DB in 1:length(CCDBType.set)) {
    # CCDBType = "ECM" # c("ECM","CC","Secret")
    CCDBType = CCDBType.set[DB]
          ...
          try({
            ...
            plt.ManyGroup2 <- plt.ManyGroup2 +
              stat_compare_means(aes(group = Cachexia),
                                 ...
                                 label.y = LabelY*1.2*0.9, show.legend = F,
