# Load Seurat_PHH_Cachexia_Integration_PBMC_04_FindCTMarkers&CTAno.RData

##### Define group by different phenotype #####
PBMC.combined$celltype <- Idents(PBMC.combined)
PBMC.combined$celltype.Cachexia <- paste(Idents(PBMC.combined), PBMC.combined$Cachexia, sep = "_")
PBMC.combined$celltype.Cachexia.gender <- paste(Idents(PBMC.combined), PBMC.combined$Cachexia, PBMC.combined$Sex, sep = "_")
Idents(PBMC.combined) <- "celltype.Cachexia.gender"

PBMC.combined$Cachexia.gender <- paste(PBMC.combined$Cachexia, PBMC.combined$Sex, sep = "_")


Idents(PBMC.combined) <- "celltype.Cachexia"
...
for(i in c(1:length(CellType.list))){ 
  try({
    Cachexia.Marker[[i]] <- Find_Markers(PBMC.combined, 
...
dir.create(paste0(PathName,"/",RVersion,"/PBMC_VolcanoPlot/"))

pdf(file = paste0(PathName,"/",RVersion,"/PBMC_VolcanoPlot.pdf"),width = 7, height = 7 )
for (i in 1:length(CellType.list)) {
  try({
    #print(Cachexia.Marker[[paste0("Cachexia.Marker.",CellType.list[i])]][["VolcanoPlot"]]+ ggtitle(paste0("PBMC_",CellType.list[i])))
    #print(Cachexia.Marker[[paste0(CellType.list[i])]][["VolcanoPlot"]]+ ggtitle(paste0("PBMC_",CellType.list[i])))
    print(VolcanoPlot(Cachexia.Marker[[i]][["Cachexia.Marker.S"]],
                      Cachexia.Marker[[i]][["Cachexia.Marker.S_Pos_List"]],
                      Cachexia.Marker[[i]][["Cachexia.Marker.S_Neg_List"]], ShowGeneNum = 6)+ ggtitle(paste0("PBMC_",CellType.list[i]))
    )
  })
}
# graphics.off()
...
for (i in 1:length(CellType.list)) {
  try({
    tiff(file = paste0(PathName,"/",RVersion,"/PBMC_VolcanoPlot/",CellType.list[i],".tif"), width = 17, height = 17, units = "cm", res = 200)
    #print(Cachexia.Marker[[paste0("Cachexia.Marker.",CellType.list[i])]][["VolcanoPlot"]]+ ggtitle(paste0("PBMC_",CellType.list[i])))
    #print(Cachexia.Marker[[paste0(CellType.list[i])]][["VolcanoPlot"]]+ ggtitle(paste0("PBMC_",CellType.list[i])))
    print(VolcanoPlot(Cachexia.Marker[[i]][["Cachexia.Marker.S"]],
                      Cachexia.Marker[[i]][["Cachexia.Marker.S_Pos_List"]],
                      Cachexia.Marker[[i]][["Cachexia.Marker.S_Neg_List"]])+ ggtitle(paste0("PBMC_",CellType.list[i]))
    )
...
write.table( CC.marker.df ,
             file = paste0(PathName,"/",RVersion,"/PBMC_CCMarker.CombineSex.tsv"),
             sep = "\t",
             quote = F,
             row.names = F
)
...
pdf(file = paste0(PathName,"/",RVersion,"/PBMC_GSEA_CombineSex.pdf"),width = 15, height = 7 )
...
write.table(GSEA.Large.Sum.TOP, file=paste0(PathName,"/",RVersion,"/Cachexia_PBMC_GSEA_Pathway_LargeTOP_CombineSex.txt"),sep="\t",
