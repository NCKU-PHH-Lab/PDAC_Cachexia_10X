...
##### Current path and new folder setting #####
  Save.Path = paste0(getwd(),"/20220208V2_SC")
...
##### Load data #####
  CCM_SPA.df <- read.delim2(paste0(Save.Path,"/SC_CCMarker_CombineSex.tsv"))
  CCM_SSA.df <- read.delim2(paste0(Save.Path,"/SC_CCMarker.tsv"))
  
  ## Modify the words
  library(stringr)
  CCM_SPA.df$Type <- str_replace(CCM_SPA.df$Type,"CCMarker_SPA.lt.","") 
  CCM_SPA.df$Type <- str_replace(CCM_SPA.df$Type,"Pos","EO") 
  CCM_SPA.df$Type <- str_replace(CCM_SPA.df$Type,"Neg","LO") 
  
  CCM_SSA.df$Type <- str_replace(CCM_SSA.df$Type,"Venn_CCMarker.","") 
...
##### Count gene number #####
  ...
  ## SSA_Intersect_All
    ...
    CCM_SSA.df[CCM_SSA.df$Intersect=="",]$Intersect <- "NA"
    CCM_SSA_Inter.set <- CCM_SSA.df$Intersect
  ...
  ## SSA_Female_All
    ...
    #CCM_SSA.df[CCM_SSA.df$Female=="",]$Female <- 0 #"NA"
    CCM_SSA_F.set <- CCM_SSA.df$Female
    CCM_SSA_F_SumCT.df <- CCMTable(CCM_SSA_F_CT.set, CCM_SSA_F.set)
    
  ## SSA_Male_All
    CCM_SSA_M_CT.set <- CCM_SSA.df[,1]
    #CCM_SSA.df[CCM_SSA.df$Male=="",]$Male <- 0  #"NA"
    CCM_SSA_M.set <- CCM_SSA.df$Male
...
##### Combine two dataframe #####
  ...
  All_Pheno.df <- CCM_All_SumCT.df %>% t() %>% as.data.frame()
  All_Pheno.df <- data.frame(Pheno = row.names(All_Pheno.df),All_Pheno.df)
  
  All_PhenoSplit.df <- str_split_fixed(All_Pheno.df$Pheno, "_", 3)
  All_Pheno.df <- data.frame(All_PhenoSplit.df,All_Pheno.df)
  colnames(All_Pheno.df) <- c("Pheno1","Pheno2","Pheno3",
                              colnames(All_Pheno.df)[4:ncol(All_Pheno.df)])
    
  write.csv( All_Pheno.df ,
